...
def register():
    conn=connector.connect(host='127.0.0.1',port='3306',user='root',password='Yashanpal6^',database='registrationform')
    mycursor=conn.cursor()
    fname=a1.get()
    lname=b1.get()
    email=c1.get()
    contact=e1.get()
    mycursor.execute("insert into module values(%s,%s,%s,%s)",(fname,lname,email,contact))
